...
@endif
    ...
    <form action="{{ route('manage-admin.update', $admin->id_admin) }}" method="POST">
        @csrf
        @method('PUT')
        
        <div style="margin-bottom: 1rem;">
            <label style="display: block; margin-bottom: 0.5rem; color: #4b5563;">Nama Admin</label>
            <input type="text" name="nama" value="{{ $admin->nama_lengkap }}" required style="width: 100%; padding: 0.5rem; border: 1px solid #d1d5db; border-radius: 6px;">
        </div>

        <div style="margin-bottom: 1rem;">
            <label style="display: block; margin-bottom: 0.5rem; color: #4b5563;">Email Admin</label>
            <input type="email" name="email" value="{{ $admin->email }}" required style="width: 100%; padding: 0.5rem; border: 1px solid #d1d5db; border-radius: 6px;">
        </div>
